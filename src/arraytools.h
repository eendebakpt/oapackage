...
inline cperm
vstack (const cperm & A, const cperm & B)
{
  cperm
  c (A.size () + B.size ());
...
void
perform_column_permutation (const array_link source, array_link & target,
...
arraydata_t
arraylink2arraydata (const array_link & al, int extracols = 0, int strength =
		     2);
 ...
 */
inline int
predictJ (const array_t * array, const int N, const int k)
{
  int t = N / 4;
  int tt = t / 2;
...
/// helper function
inline void
fastJupdate (const array_t * array, rowindex_t N, const int J,
	     const colindex_t * pp, array_t * tmp)
{
  //int jval=0;
...
/// calculate J-value for an array
int jvaluefast (const array_t * array, rowindex_t N, const int J,
		const colindex_t * pp);

/// Analyse a list of arrays
std::vector < jstruct_t > analyseArrays (const arraylist_t & arraylist,
      ...
      if (this->isopen ())
	{
	  ...
	  switch (mode)
	    ...
	    case ALATEX:
	      modestr = "latex";
	      break;
	      case ATEXT:modestr = "text";
	      break;
	      case ABINARY:modestr = "binary";
	      break;
	      case ABINARY_DIFF:modestr = "binary_diff";
	      break;
	      case ABINARY_DIFFZERO:modestr = "binary_diffzero";
	      break;
	      case AERROR:modestr = "invalid";
	      break;
	      default:modestr = "error";
