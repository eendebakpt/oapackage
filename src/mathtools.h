    ...
    object_pool()
{
  ...
  void reset() {
    while(pool.size() > 0) {
     TYPE *t = pool.back(); pool.pop_back();
     delete t;
    }
  }
    ...
    TYPE* New()
    {
	 ...
	 TYPE *t = pool.back(); pool.pop_back(); return t; }
...
public:
...
#ifdef OADEBUG
	  if(nn<=0) printf("larray: constructor from pointer: nn %d\n", nn);
#endif
        d=0;
        alloc(nn);
    ...
    larray(int nn) {
      #ifdef OADEBUG
	  if(nn<=0) printf("larray: constructor nn %d\n", nn);
#endif
        d=0;
        alloc(nn);
    }
    ...
    /*
    larray(const larray &&rhs) {
      printf("larray::constructor: rvalue reference\n");
      if(rhs.n<0) { 
	//printf("larray: copy constructor: rhs.n<0! %d\n", rhs.n);
	//this->d[-40]=0;
	this->n=-1; this->d=0; return;
    ...
    */
    ...
    bool operator== ( const larray &rhs ) const
    {
        ...
        for(int i=0; i<n; i++) {
            if(this->d[i]!=rhs.d[i]) return false;
        }
        return true;
    }
