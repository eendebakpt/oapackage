    ...
    perm_return_type permsize() const
    {
        ...
        for (int i = 0; i < ngroups; i++)
        {
            ...
            if (f != 0 && (std::numeric_limits<perm_return_type>::max() / f) < s)
            {
                ...
                myprintf("symmetry_group::permsize: group size outside limits, please use permsize_large\n");
                throw std::runtime_error("symmetry_group::permsize: group size outside limits, please use permsize_large");
                return -1;
            }
            s *= f;
        }
