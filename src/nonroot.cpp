...
#ifdef OADEBUG
    if ( src->N!=dest->N ) {
        printf ( "error: source and target dyndata_t have unequal number of rows!\n" );
        exit ( 0 );
    }
#endif
...
inline void cpy_dyndata_rowsortl ( const dyndata_t *src, dyndata_t *dest )
{
    ...
    if(src->rowsortl==0 ) {
        printf("cpy_dyndata_rowsortl: ERROR: source %ld\n", (long)src->rowsortl);
        exit(0);
    }
    if(dest->rowsortl==0 ) {
        printf("cpy_dyndata_rowsortl: ERROR: target %ld\n", (long)dest->rowsortl);
        exit(0);
        return;
    }
#endif
...
inline lmc_t LMC_check_col_tplus ( const array_t *original, const array_t *array, levelperm_t lperm, const arraydata_t *ad, const dyndata_t *dd )
{
    ...
    for ( int j = 0; j < ( nrows/oaindex ); j++ ) {
        ...
        if ( j<1 ) {
            ...
            //if (j>=1 && !issorted(rowsort+ ( j*oaindex ), rowsort+ ( j*oaindex +oaindex ) ) ) {
            // print_rowsort(rowsort, ad->N);
            //}
            ...
            for ( int k = 0; k < oaindex; k++ ) {
                ...
                if ( original[cur_row ] < lperm[array[rowp]] ) {
                    ...
                    ret = LMC_LESS;
                    //printf("ret: %d, cur_row %d= j(%d)*oaindex(%d)+k(%d);", ret, cur_row,j,oaindex,k);
            ...
            if ( ret!=LMC_EQUAL ) {
                //if (j>=1) {  printf("error: special case t+1 column!\n");   }
                break;
            }
        }
    }
...
/*
inline lmc_t LMC_check_col_tplusplus ( const array_t *original, carray_t *array, levelperm_t lperm, const arraydata_t *ad, const dyndata_t *dd ) {
    assert(ad->s[0]==2);
    lmc_t ret = LMC_EQUAL;
    int cur_row, rowp;
    const int oaindex = ad->oaindex;
...
inline lmc_t checkLMChelperSorted(int ix, int iy, const array_t *originalcol, const array_t *arraycol, levelperm_t lperm, const arraydata_t *ad,  rowsortlight_t *rowsort )
{
	...
	if(prevval==1 && cval==0) {
	  if (0) {
	  printf("checkLMChelperSorted line %d: %d to %d: ", __LINE__, ix, iy);
	 for(int v=ix; v<=iy; v++) {
	       int rowp = rowsort[v];
	   printf("%d ", lperm[arraycol[rowp]]);
	 }
	 printf("\n");
	  }
	  return LMC_LESS;
...
prevval=cval;
    ...
    if (v1==v2) return LMC_EQUAL;
    if (v1<v2) return LMC_MORE;
    return LMC_LESS;
}
...
//printf("sortzerooneR: size %d, low %d high %d\n", 1+high-low, low, high);
...
//printf("sortzeroone: size %d, low %d high %d\n", 1+high-low, low, high);

/*    if( (high-low) <=1) {
    if( (high-low) ==0)      return;
        if ( arraycol[rs[high]] < arraycol[rs[low]] ) {
            std::swap( rs[high], rs[low] );
    }   return;
} */
...
//printf("sortzerooneR: low %d high %d\n", low, high);
    ...
    if (lperm[0]==0) {
        ...
        for ( int j = 0; j < nb; j++ ) {
            ...
            if (x1==x2 && 0) {
                printf("\n### \n" );
                sd.ft.showarray();
                printf("calling sortzerooneR: scol %d, j %d, x1 %d, x2 %d\n", scol, j, x1, x2);
            }
...
lmc_t LMC_check_col_ft_testing ( const array_t *originalcol, carray_t *arraycol, levelperm_t lperm, const arraydata_t *ad, const dyndata_t *dd, const symmdata &sd, int dverbose )
{
    ...
    for ( int j = 0; j < nb; j++ ) {
        ...
        int x1 = sdp[2*j];
        int x2 = sdp[2*j+1];
        //if (col>4) 	    printf("  calling checkLMChelper: %d to %d (inclusive)\n", x1, x2-1);
...
//	      }
                ...
                if (newalg) {
                    ...
                    for ( int kk = previndex; kk < k; kk++ ) {
                        ...
                        if ( originalcol[cur_row2 ] != lperm[arraycol[rowp2]] ) {
                            ...
                            if ( originalcol[cur_row2 ] < lperm[arraycol[rowp2]] ) {
                                if (dverbose)  {
                                    printf("early abort MORE: j %d, k %d, previndex %d, kk %d\n", j, k, previndex, kk);
                                    printf("  %d %d (arraycol[rowp2] %d %d)\n", originalcol[cur_row2 ] , lperm[arraycol[rowp2]], arraycol[rowp2], rowp2 );
                                }
                                // continue;
                                //if (!fh) {   rete = LMC_MORE;  fh=1;  }
                                ret=LMC_MORE;

                                //if (v!=ret) printf("huh? ret %d, v %d\n", ret, v);
                                return ret;
                                break;
                            } else if ( originalcol[cur_row2] > lperm[arraycol[rowp2]] ) {
                                //continue;
                                if (dverbose) {
                                    printf("early abort: LESS j %d, k %d, previndex %d, kk %d\n", j, k, previndex, kk);
                                }
        ...
        if (newalg) {
            ...
            if (ret!=LMC_EQUAL) {
                if (dverbose) {
                    printf("abort: j %d, k %d, row %d, ret %d\n", j, k, j*oaindex+k, ret);
                }
                break;
            }
        }
...
//   printf("LMC_check_col_j5order: check code path...\n");
    ...
    if ( 1 && dd->col>4 ) {
        ...
        if ( log_print ( DEBUG,"" ) ) {
            printf ( "  xxx col %d, jbase %d, jcol %d: colperm: ", dd->col, jbase, jcol );
            print_perm ( pp, 5 );
        }
    ...
    if ( jbase==jcol ) {
        ...
        if ( log_print ( DEBUG,"" ) ) {
            printf ( "  j5order: col %d, jbase %d, jcol %d: colperm: ", dd->col, jbase, jcol );
            print_perm ( pp, 5 );
        }
  ...
  //dyndata[-100]=0; exit(0); // trick to get feedback from valgrind
    ...
    for ( int i=0; i<ncolsremgroup; i++ ) {
        ...
        for ( int j=0; j<nlevelperms; j++ ) {
            ...
            // printf("\n# lperm loop j=%d orignal (cpoffset %d):\n", j, cpoffset); show_array(original, ad->ncols, ad->N);
            //printf("reduction array:\n"); show_array(reduction->array, ad->ncols, ad->N);
...
//print_column_rowsort(reduction->array+dyndata->col*+ad->N, dyndata->rowsort, ad->N);
         ...
         //   debug_print_xxx ( ret, cpoffset, nlevels, dyndata, lperm, reduction, original, ad );
            ...
            if ( reduction->mode>=OA_REDUCE && ret==LMC_LESS ) {
                //printf("LMCreduce_non_root: found reduction\n");
                ...
                if (0 && reduction->mode==OA_REDUCE_PARTIAL) {
                    ...
                    if (col<reduction->targetcol) {
                        printf("  LMC_REDUCE_PARTIAL: abort loop!\n");
                        ret=LMC_LESS;
                    }
                }
            }
...
lmc_t LMCreduce_non_root_2level ( const array_t * original, const arraydata_t* ad, dyndata_t *dyndata, LMCreduction_t *reduction, const OAextend &oaextend, const LMC_static_struct_t &tmpStatic )
{
  ...
  if (dverbose)
    printf("LMCreduce_non_root_2level: level %d\n", dyndata->col);
	...
	std::swap(dyndatacpy->colperm[col], dyndatacpy->colperm[col+i]);
        ...
        lperm[0]=0; lperm[1]=1; // special init for 2-level arrays
        const int cpoffset = ad->N*dyndatacpy->colperm[dyndata->col];
        ...
        for ( int j=0; j<nlevelperms; j++ ) {
		  ...
		  //  printf("LMCreduce_non_root_2level: special check for final column: col %d (ncols %d) return %d!!!!\n", dyndatacpy->col, ad->ncols, ret);
...
#define OADEBUGL
	 ...
	 switch(col) {
	  ...
	  default:
            ...
            if ( ret==LMC_EQUAL ) {
                ...
                printf("early brake: col %d\n", dyndata->col);
                	...
                	if (specialf) {
                	 ...
                	 printf("at col %d: level perm leads to same value\n", dyndata->col);
        ...
        //std::swap ( colpermloopXX[0], colpermloopXX[i] );	// swap 2 columns back
	std::swap(dyndatacpy->colperm[col], dyndatacpy->colperm[col+i]);
