...
#pragma omp critical
  {
 // printf("  function getGlobalStatic\n");
...
{
    //printfd("cleanGlobalStatic: delete %d items\n", staticDataPool.size() );
    staticDataPool.reset();
//  for(size_t jj=0; jj<staticDataPool.size(); jj++) {
    //printfd("cleanGlobalStatic: delete item %d\n", jj);
  //  delete staticDataPool.[jj];
  //}
   // staticDataPool.resize(0);
}
...
LMC_static_struct_t * getGlobalStaticIndexed(int n)
{
 ...
 if (n>=(int(globalStaticPool.size())) )
 {
  ...
  if (verbose) {
   printf("   "); globalStaticPool[n]->show();
  }
...
/*    this->col = dd.col;
    ...
    if (dd.rowsortl!=0) {
	this->rowsortl = new rowindex_t [N];
	copy_perm(dd.rowsortl, this->rowsortl, this->N );
...
void create_root_permutations_index_helper ( rowperm_t *rperms, levelperm_t *lperms, const arraydata_t *ad, int level, int &permcounter )
{
    ...
    if ( level==ad->strength ) {
        ...
        /* check result */
        // 		if ( 0 ) {
        // 			levelperm_t * lp = malloc2d<array_t> ( ( int ) ad->strength, 10 );
        // 			root_row_permutation_from_index ( permcounter, ad, lp );
        //
        // 			for ( int z=0;z<ad->strength;z++ ) {
        // 				printf ( "perms col %d: \n", z );
        // 				print_perm ( lp[z], ad->s[z] );
        // 				print_perm ( lperms[z], ad->s[z] );
        // 			}
        // 		}
...
int fastj ( const array_t *array, rowindex_t N, const int J, const colindex_t *pp )
{
    ...
    for ( int i=0; i<J; i++ ) cp[i]=array+N*pp[i];
...
int jj45split ( carray_t *array, rowindex_t N, int jj, const colperm_t comb,  const arraydata_t &ad, const OAextend &oaextend, LMC_static_struct_t &tmpStatic, LMCreduction_t &reduction, int verbose=0 )
{
 // if (checkloglevel ( DEBUG ) )
   ...
   // s.sortdescending(ww); // ????
    ...
    if (0) {
      printf("entering LMCreduce: "); dyndata.show();	// @pte
...
//	printf("reduction.sd %ld\n", (long) reduction.sd.get() );
...
//    LMC_static_struct_t *tmpStatic = getGlobalStatic();
    ...
    for(int i=ad.strength; i<ad.ncols+1-1; i++) {
        ...
        if (ad.ncols==0) printf("ad.ncols==0!\n");
...
//            std::vector<colindex_t> w = reductionsub.symmetries[i][j].colperm;  w.push_back(newcol);
	...
	// note: above call  is expensive, merge with initrowsortl call
		    ...
		    if (oaextend.getAlgorithm() ==MODE_LMC_SYMMETRY || oaextend.getAlgorithm() ==MODE_J5ORDERXFAST || oaextend.getAlgorithm() ==MODE_LMC_2LEVEL) {
		      ...
		      printf("oaextend problem: "); oaextend.info();
		      exit(0);
...
LMCreduction_t calculateSymmetryGroups(const array_link &al, const arraydata_t &adata, const OAextend &oaextend, int dverbose, int hack)
{
 ...
 // dverbose=1;
    ...
    // pre-compute
    array_link alsub = al;  arraydata_t adatasub(&adata, al.n_columns);
...
lmc_t LMCcheck ( const array_t * array, const arraydata_t &ad, const OAextend &oaextend, LMCreduction_t &reduction )
{
    ...
    case MODE_LMC_SYMMETRY:
    {
	 ...
	 //reductionsub.symms.show();
	  ...
	  //reductionsub.colcombs = reduction.colcombs;
	  
	  //  reductionsub.symmetries.push_back( symmetryset() );
	  //  reductionsub.colperms.push_back( colpermset() );
	  //  reductionsub.colcombs.push_back( colpermset() );
	}
        //printf("nx\n");
        if (dverbose) {
	...
	//dverbose=1; printf("gr: reduction.symms.store %d\n", reduction.symms.store); 	oaextend.info();

	OAextend xx =oaextend; xx.setAlgorithm(MODE_J5ORDERXFAST);
	...
	//printf("LMCcheckj5: time %.4f [ms] (ncols %d, lmc %d)\n", 1e3*(get_time_ms()-t0), al.n_columns, lmc );
...
*/
    ...
    //log_print ( DEBUG+1, "## LMC: there are %d column combinations\n", nc );
    ...
    for ( int i=0; i<nc; i++ ) {
        ...
        for ( int z=0; z<factorial<int> ( k ); z++ ) {
            ...
            //perform_inv_column_permutation ( array, cpy, localcolperm, ad->N, ad->ncols );
...
void reduceArraysGWLP ( const arraylist_t *arraylist, arraylist_t &earrays, int verbose, int dopruning, int strength, int dolmc )
{
    ...
    for ( size_t i=0; i< ( size_t ) arraylist->size(); i++ ) {
        ...
        if ( verbose>=4 ) {
            std::vector<double> gwp = GWLP ( arraylist->at ( i ) );
            cout << "GMA: ";
            printf_vector<double> ( gwp, "%.3f " );
            cout << endl;
        }
        std::vector< GWLPvalue > dopgwp = projectionGWLPs ( arraylist->at ( i ) );
      ...
      printfd("reduceArraysGWLP:\n");
        ...
        if ( verbose>=2 ) {
            ...
            //printf_vector< GWLPvalue > ( sgwp, "%.4f " );
        ...
        //lmc_t rx = LMCcheck(alf, ad, oaextend, reduction, lm, verbose);
        //lmc_t rx = reduceCanonicalGWLP ( alf, ad, oaextend, reduction, lm, verbose );
...
if (changed) {
    ...
    //extend_array_dynamic(*arraylist, *ad, oaextend, earrays,  kfinal,  Afinal,  verbose);
    if ( verbose ) {
