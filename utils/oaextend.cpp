...
AnyOption * parseOptions(int argc, char* argv[], algorithm_t &algorithm )
{
    ...
    else algorithm = MODE_AUTOSELECT;
...
int main(int argc, char* argv[])
{
	...
	const bool streaming = opt->getFlag( "streaming");
        ...
        // int maxk2 = opt->getIntValue('K', 100000);
//	printf("  grf %d %d\n", maxk, maxk2);
        ...
        OAextend oaextend;
        oaextend.setAlgorithm(algorithm);
        //printf("gr %d -> %d %s\n", algorithm, oaextend.getAlgorithm(), oaextend.getAlgorithmName().c_str() );
...
//            MPI_Bcast(&alg, 1, MPI_INT, MASTER, MPI_COMM_WORLD); /* broadcast a message */
            //MPI_Bcast(&p.N, 1, MPI_INT, MASTER, MPI_COMM_WORLD); /* broadcast a message */
#endif
    ...
    int nb = arrayfile_t::arrayNbits(*ad);
	  ...
	  oaextend.storefile.createfile(fname, adcol->N, ad->ncols, -1, ABINARY, nb);
		    ...
		    //if (cur_extension-solutions.begin()>2) break;
                ...
                free_sols(extensions);		//clear old to free up space for new extensions
