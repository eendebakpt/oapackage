 ...
 */
bool readNumbersFile(const char *numbersfile, std::vector < long >&na,
		     std::vector < long >&npareto, int kmin =
		     -1, int kmax = -1)
{
...
void writeNumbersFile(const char *numbersfile,
		      const std::vector < long >&na,
		      const std::vector < long >&npareto, int kmin =
		      -1, int kmax = -1)
{
...
int main(int argc, char *argv[])
{
...
#ifdef DOOPENMP
	printf
	    ("oaclustergather: openmp: num threads %d, max num threads %d\n",
    ...
    if (verbose) {
	std::cout << "oaclustergather: levels " << splitTag(lvls) << std::
	    endl;
    }
	...
	for (int jj = 0; jj < nsplit[level]; jj++) {
		...
		"" + (methodtag +
		      ("-" + splitTag(tovec(lvls, jj)) + ".txt"));
	    std::string nfilesub =
		basedir + filesep + subdir + filesep + nfilesub0;

	    Jcounter jck =
		readStatisticsFile(nfilesub.c_str(), verbose >= 2);
	    bool b = jck.isOpen();
		...
		if ((!b) && (nnarrays < 0)) {
		    printf
			("no numbers file and no array file (afile %s)\n",
			 afile.c_str());
	...
	if (verbose >= 1) {
	    printf("  total number of arrays: %ld, %.1f Marrays/hour\n",
		   jc.narrays(),
		   double (3600. / 1e6) * double (jc.narrays()) /
		   (get_time_ms() - time0));
	}
	...
	for (int k = kmin; k <= kmax; k++) {
		...
		bool b =
		    readNumbersFile(nfilesub.c_str(), nasub, nparetosub,
				    kmin, kmax);
		...
		if (verbose >= 2) {
			...
			printf
			    ("   --> could not read read numbers file %s\n",
			     nfilesub.c_str());
		    }
		}
		...
		const std::string parfile =
		    basedir + filesep + subdir + filesep + subfilepareto0;
		...
		if (nn >= 0)
			...
			if (verbose >= 3) {
			    printf
				("  --> both numbers file and pareto file, checking whether numbers are equal\n");
			}
			// check
			if (nparetosub[k] != nn) {
			    printfd
		...
		long naread = 0;	// number of arrays read
		{
			...
			if (verbose) {
			    myprintf
				("oaclustergather: problem with file %s\n",
			...
			if (verbose >= 2)
			    printf
				("oaclustergather: read arrays in block %d: %d arrays (%ld/%ld)\n",
				 loop, (int) arraylist.size(), naread,
				 narrays);
			if (arraylist.size() <= 0)
			    break;
		...
		if (verbose >= 3
			...
			&& verbose >= 2)) {
		    printf
			("oaclustergather: file %d/%d, %ld arrays: %d Pareto values, %d Pareto elements\n",
			 jj, nsplit[level], naread, pset.number(),
			 pset.numberindices());
		    fflush(0);
		}
	    }
	    ...
	    if (verbose) {
		printf("  final pareto set %d cols (%d files): ", k,
		       nsplit[level]);
		if (verbose >= 2)
		    pset.show(2);
		else
		...
		std::string pfile0 =
		    printfstring("%s-%s-%s.oa", xxx.c_str(), outputprefix,
				 adata0.idstr().c_str());
		if (!cleanrunK) {
	...
	const long natotal =
	    std::accumulate(na.begin(), na.end(), (long) 0);
	...
	std::cout << "#time total: " << printfstring("%.1f",
						     get_time_ms() -
						     time0) << " [s]" <<
	    std::endl;
	fflush(0);
