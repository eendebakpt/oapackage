...
def getOptimizationResults(arrayclass, alpha, nrestarts=80):
    scores, dds, designs, _ = oapackage.Doptimize(
        arrayclass, nrestarts=nrestarts, optimfunc=alpha, selectpareto=False)
    if 1:
...
for jj, result in enumerate(results):
    ...
    if optim_func[1] > 0:
        if optim_func[1]==1:
            ...
            lbl += ' + %g$D_s$' % optim_func[1]
    if optim_func[2] > 0:
        lbl += (' + %g$D_{s0}$' % optim_func[2])
    lbls += [lbl]

data = data[[0, 1, -1], :]
x = oapackage.generateDscatter(
    data.T, lbls=lbls, fig=30, ndata=data.shape[0] - 1, scatterarea=80)
...
if arrayclass.N == 40:
    plt.gca().scatter(ddst3[:, 1], ddst3[:, 0], marker='s', s=90, c=(
        0, 0, 0), linewidths=0, alpha=1, label='Strength 3')
    xl = plt.xlim()
    plt.xlim([xl[0], 1.05])
    yl = plt.ylim()
    plt.ylim([.78, yl[1]])
